...
public class UploadFileUtils {
	...
	public static String uploadFile(MultipartFile file, HttpServletRequest request) throws Exception {
		...
		// 4. 파일 복사
		File target = new File(rootPath + datePath, uuidFileName); // 경로, 파일 객체
		FileCopyUtils.copy(fileData, target); // 생성된 타겟에 바이트 스트림 복사
		
		// 5. 이미지 썸네일 생성
		uuidFileName = makeThumbnailImg(rootPath, datePath, uuidFileName);
	...
	public static String getRootPath(HttpServletRequest request) {
		...
		return request.getSession().getServletContext().getRealPath(rootPath + "/media");
